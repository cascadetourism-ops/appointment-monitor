...
POLL_INTERVAL_BASE = int(os.environ.get("POLL_INTERVAL_BASE", 60))
...
def check_location_slots(location_name, facility_id, headers):
    ...
    try:
        ...
        # -----------------------------------------------------------------
        # LIVE API REQUEST IMPLEMENTATION (MOCKED FOR DEMO)
        # -----------------------------------------------------------------
        ...
        #     send_telegram_alert("⚠️ Critical: Session cookies have expired or been blocked! Update required.")
        ...
        # Mock results representation
        if location_name == "Dubai":
            earliest_date_str = "2026-10-20"
            total_slots = 3
        else:
...
def continuous_scheduler():
    global is_running
    logging.info("Background Monitoring Thread Initialized.")
    while is_running:
        monitor_appointment_dates()
        
        base_sleep = random.uniform(POLL_INTERVAL_BASE - 5, POLL_INTERVAL_BASE + 15)
        total_sleep = base_sleep * backoff_multiplier
        
        logging.info(f"Sleeping for {round(total_sleep, 2)} seconds before next cycle...")
        
        elapsed = 0
        while elapsed < total_sleep and is_running:
            time.sleep(1)
            elapsed += 1
